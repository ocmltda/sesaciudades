<?php
	...
	{
		if ($_GET['rok'] == 1)
			$msjaddcarga = '<font color="green"><strong>Acción realizada correctamente.</strong></font>';
		else
			$msjaddcarga = '<font color="red"><strong>Hubo un error en la acción realizada.</strong></font>';
	}
	else
		$msjaddcarga = '';
?>
	...
	if (isset($_SESSION['xmlSession']) && isset($_SESSION['rut']) && isset($_SESSION['dv']))
	{
		...
		if (!isset($xml->error))
		{
			foreach($xml->list->Carga as $val)
			{
				...
				echo '<td><a href="updatecarga.php?rut=' . $val->rutCarga . '">Modificar</a> | <a href="delcarga.php?rut=' . $val->rutCarga . '" onclick="return confirmar(\'¿Está seguro que desea eliminar la carga de rut ' . $val->rutCarga . '?\')">Eliminar</a></td>';
				echo '</tr>';
			}
		}
		else
