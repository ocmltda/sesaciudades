<?php
	...
	if (isset($_POST['doIngresar']))
	{
		if ($_POST['doIngresar']=='Ingresar')
		{
			...
			//llamo a la funcion de login que me retorna el xmlSession, con los datos del formulario
			$result = $client->getSessionId(array("rut" => $arrRut[0], "digVer" => $arrRut[1], "password" => $clave));
			...
			$xmlSession = trim($response_arr["getSessionIdReturn"]) . '';
			$xml = simplexml_load_string($xmlSession);

			//recupero las variables del xml
			$idSession = $xml->idSession;
			$isValid = $xml->isValid;
			$fechaCreacion = $xml->fechaCreacion;
			$id = $xml->id;

			//si me logeo OK, la variable isValid es 'true'
			if ($isValid == 'true')
			{
				newup_var_sess('xmlSession', $xmlSession);
				newup_var_sess('rut', $rut);
				newup_var_sess('dv', $dv);
			...
			else
			{
				...
				$err[] = "Error - Los datos ingresado son incorrectos.";
			}
		}
	}
?>
    ...
    <td align="left" valign="top" bgcolor="#B54515"  style="padding-left:15px;">
	  ...
	  **************************************************************************/
      ...
      <form action="login.php" method="post" name="logForm" id="logForm" >
        <input name="rf" type="hidden" id="rf" value="<?php echo $_GET['rf']; ?>" />
        <table width="65%" border="0" align="center" cellpadding="4" cellspacing="4" class="form">
          <tr> 
            <td colspan="2" class="titulosNuestraClinica2">Ingresar a mi Cuenta<br>
              <em><span class="txt">Ingresar datos tal cual se registraron (respetando las may&uacute;sculas o min&uacute;sculas que registraste)</span></em></td>
          </tr>
          <tr> 
            <td width="28%" class="titulosNuestraClinica">Rut</td>
            <td width="72%"><input name="usr_email" type="text" class="required" id="txtbox" size="25"></td>
          </tr>
          <tr> 
            <td class="titulosNuestraClinica">Contrase&ntilde;a</td>
            <td><input name="pwd" type="password" class="required password" id="txtbox" size="25"></td>
          </tr>
          <tr> 
            <td colspan="2"><div align="center">
                <input name="remember" type="checkbox" id="remember" value="1">
                <span class="txt">Recordar Contrase&ntilde;a</span></div></td>
          </tr>
          <tr> 
            <td colspan="2"> <div align="center"> 
                <p> 
                ...
                <p><a href="register.php">Registrarse</a><font color="#FF6600"> 
                  |</font> <a href="forgot.php">Recuperar contraseña</a> <font color="#FF6600"> 
                  </font></p>
...
<!--
    ...
    tagCertifica( 25780, '/home/beneficios/ingreso');
