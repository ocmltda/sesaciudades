...
// Create page object
$local_edit = new clocal_edit();
$Page =& $local_edit;

// Page init
$local_edit->Page_Init();

// Page main
$local_edit->Page_Main();
?>
...
// Create page object
var local_edit = new ew_Page("local_edit");

// page properties
local_edit.PageID = "edit"; // page ID
local_edit.FormID = "flocaledit"; // form ID
var EW_PAGE_ID = local_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
local_edit.ValidateForm = function(fobj) {
...
local_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
local_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
local_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
local_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $local_edit->ShowPageHeader(); ?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $local->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $local->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php
$local_edit->ShowMessage();
?>
<form name="flocaledit" id="flocaledit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return local_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="local">
...
<?php if ($local->loc_id->Visible) { // loc_id ?>
	<tr id="r_loc_id"<?php echo $local->RowAttributes() ?>>
		...
		<td<?php echo $local->loc_id->CellAttributes() ?>><span id="el_loc_id">
<div<?php echo $local->loc_id->ViewAttributes() ?>><?php echo $local->loc_id->EditValue ?></div>
<input type="hidden" name="x_loc_id" id="x_loc_id" value="<?php echo ew_HtmlEncode($local->loc_id->CurrentValue) ?>">
</span><?php echo $local->loc_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php if ($local->loc_comuna->Visible) { // loc_comuna ?>
	<tr id="r_loc_comuna"<?php echo $local->RowAttributes() ?>>
		...
		<td<?php echo $local->loc_comuna->CellAttributes() ?>><span id="el_loc_comuna">
<input type="text" name="x_loc_comuna" id="x_loc_comuna" size="30" maxlength="32" value="<?php echo $local->loc_comuna->EditValue ?>"<?php echo $local->loc_comuna->EditAttributes() ?>>
</span><?php echo $local->loc_comuna->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($local->loc_ciudad->Visible) { // loc_ciudad ?>
	<tr id="r_loc_ciudad"<?php echo $local->RowAttributes() ?>>
		...
		<td<?php echo $local->loc_ciudad->CellAttributes() ?>><span id="el_loc_ciudad">
<input type="text" name="x_loc_ciudad" id="x_loc_ciudad" size="30" maxlength="10" value="<?php echo $local->loc_ciudad->EditValue ?>"<?php echo $local->loc_ciudad->EditAttributes() ?>>
</span><?php echo $local->loc_ciudad->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</div>
...
class clocal_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$local->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$local->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$local->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$local->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($local->loc_id->CurrentValue == "")
			$this->Page_Terminate("locallist.php"); // Invalid key, return to list
		switch ($local->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$local->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$local->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$local->ResetAttrs();
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		if (!$local->loc_comuna->FldIsDetailKey) {
			$local->loc_comuna->setFormValue($objForm->GetValue("x_loc_comuna"));
		}
		if (!$local->loc_ciudad->FldIsDetailKey) {
			$local->loc_ciudad->setFormValue($objForm->GetValue("x_loc_ciudad"));
		}
	}
	...
	function RestoreFormValues() {
		...
		$local->loc_comuna->CurrentValue = $local->loc_comuna->FormValue;
		$local->loc_ciudad->CurrentValue = $local->loc_ciudad->FormValue;
	}
	...
	function RenderRow() {
		...
		if ($local->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// loc_comuna
			$local->loc_comuna->EditCustomAttributes = "";
			$local->loc_comuna->EditValue = ew_HtmlEncode($local->loc_comuna->CurrentValue);

			// loc_ciudad
			$local->loc_ciudad->EditCustomAttributes = "";
			$local->loc_ciudad->EditValue = ew_HtmlEncode($local->loc_ciudad->CurrentValue);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// loc_comuna
			$local->loc_comuna->SetDbValueDef($rsnew, $local->loc_comuna->CurrentValue, NULL, FALSE);

			// loc_ciudad
			$local->loc_ciudad->SetDbValueDef($rsnew, $local->loc_ciudad->CurrentValue, NULL, FALSE);
