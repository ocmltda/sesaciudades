...
// Create page object
$categoria_cat_imagen_nombre_blobview = new ccategoria_cat_imagen_nombre_blobview();
$Page =& $categoria_cat_imagen_nombre_blobview;

// Page init
$categoria_cat_imagen_nombre_blobview->Page_Init();

// Page main
$categoria_cat_imagen_nombre_blobview->Page_Main();
?>
<?php
$categoria_cat_imagen_nombre_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class ccategoria_cat_imagen_nombre_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'categoria_cat_imagen_nombre_blobview';
	...
	//
	function ccategoria_cat_imagen_nombre_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('categoria', 'x_cat_imagen_nombre');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				if (trim(strval($this->Recordset->fields('cat_imagen_tipo'))) <> "") {
					header("Content-type: ". $this->Recordset->fields('cat_imagen_tipo'));
				} else {
					if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					 header("Content-type: images");
				}
				if (trim(strval($this->Recordset->fields('cat_imagen_nombre'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $this->Recordset->fields('cat_imagen_nombre'));
				}
				$objBinary->Value = $this->Recordset->fields('cat_imagen_nombre');
