...
class clocal_grid {
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_loc_vigente") && $objForm->HasValue("o_loc_vigente") && $local->loc_vigente->CurrentValue <> $local->loc_vigente->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_loc_comuna") && $objForm->HasValue("o_loc_comuna") && $local->loc_comuna->CurrentValue <> $local->loc_comuna->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_loc_ciudad") && $objForm->HasValue("o_loc_ciudad") && $local->loc_ciudad->CurrentValue <> $local->loc_ciudad->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$local->loc_comuna->setOldValue($objForm->GetValue("o_loc_comuna"));
		if (!$local->loc_ciudad->FldIsDetailKey) {
			$local->loc_ciudad->setFormValue($objForm->GetValue("x_loc_ciudad"));
		}
		$local->loc_ciudad->setOldValue($objForm->GetValue("o_loc_ciudad"));
	}
