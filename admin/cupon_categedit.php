...
// Create page object
$cupon_categ_edit = new ccupon_categ_edit();
$Page =& $cupon_categ_edit;

// Page init
$cupon_categ_edit->Page_Init();

// Page main
$cupon_categ_edit->Page_Main();
?>
...
// Create page object
var cupon_categ_edit = new ew_Page("cupon_categ_edit");

// page properties
cupon_categ_edit.PageID = "edit"; // page ID
cupon_categ_edit.FormID = "fcupon_categedit"; // form ID
var EW_PAGE_ID = cupon_categ_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
cupon_categ_edit.ValidateForm = function(fobj) {
...
cupon_categ_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
cupon_categ_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
cupon_categ_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
cupon_categ_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $cupon_categ_edit->ShowPageHeader(); ?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $cupon_categ->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $cupon_categ->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php
$cupon_categ_edit->ShowMessage();
?>
<form name="fcupon_categedit" id="fcupon_categedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return cupon_categ_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="cupon_categ">
...
<?php if ($cupon_categ->cct_id->Visible) { // cct_id ?>
	<tr id="r_cct_id"<?php echo $cupon_categ->RowAttributes() ?>>
		...
		<td<?php echo $cupon_categ->cct_id->CellAttributes() ?>><span id="el_cct_id">
<div<?php echo $cupon_categ->cct_id->ViewAttributes() ?>><?php echo $cupon_categ->cct_id->EditValue ?></div>
<input type="hidden" name="x_cct_id" id="x_cct_id" value="<?php echo ew_HtmlEncode($cupon_categ->cct_id->CurrentValue) ?>">
</span><?php echo $cupon_categ->cct_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$cupon_categ_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$cupon_categ_edit->Page_Terminate();
?>
<?php
...
class ccupon_categ_edit {
	...
	// Page object name
	var $PageObjName = 'cupon_categ_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$cupon_categ->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$cupon_categ->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$cupon_categ->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$cupon_categ->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($cupon_categ->cct_id->CurrentValue == "")
			$this->Page_Terminate("cupon_categlist.php"); // Invalid key, return to list
		switch ($cupon_categ->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$cupon_categ->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$cupon_categ->RowType = EW_ROWTYPE_EDIT; // Render as Edit
