...
// Create page object
$promocion_pro_imagen_nombre_blobview = new cpromocion_pro_imagen_nombre_blobview();
$Page =& $promocion_pro_imagen_nombre_blobview;

// Page init
$promocion_pro_imagen_nombre_blobview->Page_Init();

// Page main
$promocion_pro_imagen_nombre_blobview->Page_Main();
?>
<?php
$promocion_pro_imagen_nombre_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class cpromocion_pro_imagen_nombre_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'promocion_pro_imagen_nombre_blobview';
	...
	//
	function cpromocion_pro_imagen_nombre_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('promocion', 'x_pro_imagen_nombre');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				if (trim(strval($this->Recordset->fields('pro_imagen_tipo'))) <> "") {
					header("Content-type: ". $this->Recordset->fields('pro_imagen_tipo'));
				} else {
					if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					 header("Content-type: images");
				}
				if (trim(strval($this->Recordset->fields('pro_imagen_nombre'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $this->Recordset->fields('pro_imagen_nombre'));
				}
				$objBinary->Value = $this->Recordset->fields('pro_imagen_nombre');
