...
class clocal {
	...
	function clocal() {
		...
		// loc_comuna
		$this->loc_comuna = new cField('local', 'local', 'x_loc_comuna', 'loc_comuna', '`loc_comuna`', 200, -1, FALSE, '`loc_comuna`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['loc_comuna'] =& $this->loc_comuna;

		// loc_ciudad
		$this->loc_ciudad = new cField('local', 'local', 'x_loc_ciudad', 'loc_ciudad', '`loc_ciudad`', 200, -1, FALSE, '`loc_ciudad`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['loc_ciudad'] =& $this->loc_ciudad;
	}
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->loc_comuna->setDbValue($rs->fields('loc_comuna'));
		$this->loc_ciudad->setDbValue($rs->fields('loc_ciudad'));
	}
   ...
   // Common render codes
		...
		// loc_comuna
		$this->loc_comuna->ViewValue = $this->loc_comuna->CurrentValue;
		$this->loc_comuna->ViewCustomAttributes = "";

		// loc_ciudad
		$this->loc_ciudad->ViewValue = $this->loc_ciudad->CurrentValue;
		$this->loc_ciudad->ViewCustomAttributes = "";
		...
		// loc_comuna
		$this->loc_comuna->LinkCustomAttributes = "";
		$this->loc_comuna->HrefValue = "";
		$this->loc_comuna->TooltipValue = "";

		// loc_ciudad
		$this->loc_ciudad->LinkCustomAttributes = "";
		$this->loc_ciudad->HrefValue = "";
		$this->loc_ciudad->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('loc_comuna', $this->loc_comuna->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('loc_ciudad', $this->loc_ciudad->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('loc_id', $this->loc_id->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('emp_id', $this->emp_id->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('loc_nombre', $this->loc_nombre->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->loc_comuna);
				$Doc->ExportCaption($this->loc_ciudad);
			} else {
				$Doc->ExportCaption($this->loc_id);
				$Doc->ExportCaption($this->emp_id);
				$Doc->ExportCaption($this->loc_nombre);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->loc_comuna);
					$Doc->ExportField($this->loc_ciudad);
				} else {
					$Doc->ExportField($this->loc_id);
					$Doc->ExportField($this->emp_id);
					$Doc->ExportField($this->loc_nombre);
