...
<p>
<?php if ($cupon->getCurrentDetailTable() == "cupon_categ" && $cupon_categ->DetailAdd) { ?>
<br>
<?php include_once "cupon_categgrid.php" ?>
<br>
<?php } ?>
<?php if ($cupon->getCurrentDetailTable() == "cupon_region" && $cupon_region->DetailAdd) { ?>
<br>
...
class ccupon_add {
		...
		switch ($cupon->CurrentAction) {
			...
			case "A": // ' Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if ($cupon->getCurrentDetailTable() <> "") // Master/detail add
	...
	function ValidateForm() {
		...
		if ($cupon->getCurrentDetailTable() == "cupon_categ" && $GLOBALS["cupon_categ"]->DetailAdd) {
			$cupon_categ_grid = new ccupon_categ_grid(); // get detail page object
			$cupon_categ_grid->ValidateGridForm();
			$cupon_categ_grid = NULL;
		}
		if ($cupon->getCurrentDetailTable() == "cupon_region" && $GLOBALS["cupon_region"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			if ($cupon->getCurrentDetailTable() == "cupon_categ" && $GLOBALS["cupon_categ"]->DetailAdd) {
				$GLOBALS["cupon_categ"]->cup_id->setSessionValue($cupon->cup_id->CurrentValue); // Set master key
				$cupon_categ_grid = new ccupon_categ_grid(); // get detail page object
				$AddRow = $cupon_categ_grid->GridInsert();
				$cupon_categ_grid = NULL;
			}
			if ($cupon->getCurrentDetailTable() == "cupon_region" && $GLOBALS["cupon_region"]->DetailAdd) {
				$GLOBALS["cupon_region"]->cup_id->setSessionValue($cupon->cup_id->CurrentValue); // Set master key
				$cupon_region_grid = new ccupon_region_grid(); // get detail page object
				$AddRow = $cupon_region_grid->GridInsert();
				$cupon_region_grid = NULL;
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			if ($sDetailTblVar == "cupon_categ") {
				...
				if ($GLOBALS["cupon_categ"]->DetailAdd) {
					if ($cupon->CurrentAction == "C")
						$GLOBALS["cupon_categ"]->CurrentMode = "copy";
					else
						$GLOBALS["cupon_categ"]->CurrentMode = "add";
					$GLOBALS["cupon_categ"]->CurrentAction = "gridadd";
			...
			if ($sDetailTblVar == "cupon_region") {
				...
				if ($GLOBALS["cupon_region"]->DetailAdd) {
					if ($cupon->CurrentAction == "C")
						$GLOBALS["cupon_region"]->CurrentMode = "copy";
					else
						$GLOBALS["cupon_region"]->CurrentMode = "add";
					$GLOBALS["cupon_region"]->CurrentAction = "gridadd";
