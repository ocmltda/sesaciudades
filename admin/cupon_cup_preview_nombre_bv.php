...
// Create page object
$cupon_cup_preview_nombre_blobview = new ccupon_cup_preview_nombre_blobview();
$Page =& $cupon_cup_preview_nombre_blobview;

// Page init
$cupon_cup_preview_nombre_blobview->Page_Init();

// Page main
$cupon_cup_preview_nombre_blobview->Page_Main();
?>
<?php
$cupon_cup_preview_nombre_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class ccupon_cup_preview_nombre_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'cupon_cup_preview_nombre_blobview';
	...
	//
	function ccupon_cup_preview_nombre_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('cupon', 'x_cup_preview_nombre');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				if (trim(strval($this->Recordset->fields('cup_preview_tipo'))) <> "") {
					header("Content-type: ". $this->Recordset->fields('cup_preview_tipo'));
				} else {
					if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					 header("Content-type: images");
				}
				if (trim(strval($this->Recordset->fields('cup_preview_nombre'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $this->Recordset->fields('cup_preview_nombre'));
				}
				$objBinary->Value = $this->Recordset->fields('cup_preview_nombre');
