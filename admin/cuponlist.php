...
<?php if ($cupon_categ->DetailAdd && $Security->IsLoggedIn()) { ?>
<a href="<?php echo $cupon->AddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=cupon_categ" ?>"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $cupon->TableCaption() ?>/<?php echo $cupon_categ->TableCaption() ?></a>&nbsp;&nbsp;
<?php } ?>
<?php if ($cupon_region->DetailAdd && $Security->IsLoggedIn()) { ?>
<a href="<?php echo $cupon->AddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=cupon_region" ?>"><?php echo $Language->Phrase("AddLink") ?>&nbsp;<?php echo $cupon->TableCaption() ?>/<?php echo $cupon_region->TableCaption() ?></a>&nbsp;&nbsp;
<?php } ?>
<?php } ?>
...
class ccupon_list {
	...
	function ccupon_list() {
		...
		$this->AddUrl = "cuponadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function SetupListOptions() {
		...
		// "detail_cupon_categ"
		$item =& $this->ListOptions->Add("detail_cupon_categ");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->IsLoggedIn();
		$item->OnLeft = TRUE;

		// "detail_cupon_region"
		$item =& $this->ListOptions->Add("detail_cupon_region");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->IsLoggedIn();
		$item->OnLeft = TRUE;
	...
	function RenderListOptions() {
		...
		// "detail_cupon_categ"
		$oListOpt =& $this->ListOptions->Items["detail_cupon_categ"];
		if ($Security->IsLoggedIn()) {
			$oListOpt->Body = $Language->Phrase("DetailLink") . $Language->TablePhrase("cupon_categ", "TblCaption");
			$oListOpt->Body = "<a href=\"cupon_categlist.php?" . EW_TABLE_SHOW_MASTER . "=cupon&cup_id=" . urlencode(strval($cupon->cup_id->CurrentValue)) . "\">" . $oListOpt->Body . "</a>";
			$links = "";
			if ($GLOBALS["cupon_categ"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn())
				$links .= "<a href=\"" . $cupon->EditUrl(EW_TABLE_SHOW_DETAIL . "=cupon_categ") . "\">" . $Language->Phrase("EditLink") . "</a>&nbsp;";
			if ($GLOBALS["cupon_categ"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn())
				$links .= "<a href=\"" . $cupon->CopyUrl(EW_TABLE_SHOW_DETAIL . "=cupon_categ") . "\">" . $Language->Phrase("CopyLink") . "</a>&nbsp;";
			if ($links <> "") $oListOpt->Body .= "<br>" . $links;
		}

		// "detail_cupon_region"
		$oListOpt =& $this->ListOptions->Items["detail_cupon_region"];
		if ($Security->IsLoggedIn()) {
			$oListOpt->Body = $Language->Phrase("DetailLink") . $Language->TablePhrase("cupon_region", "TblCaption");
			$oListOpt->Body = "<a href=\"cupon_regionlist.php?" . EW_TABLE_SHOW_MASTER . "=cupon&cup_id=" . urlencode(strval($cupon->cup_id->CurrentValue)) . "\">" . $oListOpt->Body . "</a>";
			$links = "";
			if ($GLOBALS["cupon_region"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn())
				$links .= "<a href=\"" . $cupon->EditUrl(EW_TABLE_SHOW_DETAIL . "=cupon_region") . "\">" . $Language->Phrase("EditLink") . "</a>&nbsp;";
			if ($GLOBALS["cupon_region"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn())
				$links .= "<a href=\"" . $cupon->CopyUrl(EW_TABLE_SHOW_DETAIL . "=cupon_region") . "\">" . $Language->Phrase("CopyLink") . "</a>&nbsp;";
			if ($links <> "") $oListOpt->Body .= "<br>" . $links;
		}
